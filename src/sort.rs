...
impl SortOption {
    pub fn compare_func(&self) -> impl Fn(&LllDirEntry, &LllDirEntry) -> cmp::Ordering {
...
const fn dummy_dir_first(_: &LllDirEntry, _: &LllDirEntry) -> cmp::Ordering {
    cmp::Ordering::Equal
}

fn dir_first(f1: &LllDirEntry, f2: &LllDirEntry) -> cmp::Ordering {
...
fn natural_sort_case_insensitive(f1: &LllDirEntry, f2: &LllDirEntry) -> cmp::Ordering {
    let f1_name = f1.file_name().to_lowercase();
    let f2_name = f2.file_name().to_lowercase();
    compare_str(&f1_name, &f2_name)
}

fn natural_sort(f1: &LllDirEntry, f2: &LllDirEntry) -> cmp::Ordering {
    compare_str(f1.file_name(), f2.file_name())
}

fn mtime_sort(file1: &LllDirEntry, file2: &LllDirEntry) -> cmp::Ordering {
    fn compare(file1: &LllDirEntry, file2: &LllDirEntry) -> Result<cmp::Ordering, std::io::Error> {
